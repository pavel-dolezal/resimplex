...
do j=1, n_ord
		...
		alphaf = 0.d0
                ...
                chi2 = chi2 + (free_a(j)-alphaf)**2

		if (rectifinal) then
			write (666,*) j, free_a(j), alphaf
		end if	
	end if
end do ! j=n_ord

! printing current iteration to chi2_func.tmp
...
end function prefit_a_func

end module prefit_a_func_module
