...
! module creates logical array - any point inside any teluric line can then be ignored
...
integer :: ioerr, iocorrerr, i, j, k, m

print *, "Teluric module started..."
flush(6)

! number of teluric points removed
n_telpoint = 0
...
if (file_existence) then
	...
	print *, "Error: No coomon_input/heliocorr.asc file"
end if
...
print *, "common_input/teluric.asc opened"
...
Do i=1, n_dat
	...
	do j=1, n_ord
		do k=1, n_pnt
			! check if datapoint is in a teluric line
			telx1=0
			do
			200	read (80, "(A)", iostat = ioerr) line
				...
				! aply heliocentric correction to the point of the teluric spectrum
				telx2 = telx2*(new_corr/299792.458+1.d0)
				
				if (telx2 > observed_wav(i,j,k)) then
					if (telx1 < 0.01) then
						print *, "Error: 1st point of common_input/teluric.asc > 1st data point; (i, j, k, telx1) = ", i, j, k, telx1
						stop
					else
						exit
					end if
