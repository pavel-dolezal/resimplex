...
do j=1, n_ord
	...
	if (((skipped_orders(j) == 0).or.(rectifinal)).and.((j>min2).and.(j<=max2))) then
		...
		max_blazef = free_mb(dataset,j)

		if (rectifinal) then
			write (31,*) i,j,lambda_cf
		end if

                if (max_blazef < 0.0) then
                        chi2 = chi2 + 100000*abs(max_blazef)
                end if
		...
		do k=1, n_pnt	!across points
			...
			if (teluric(i,j,k)) then
				...
				if (debug_chi2_funcA) then
					...
					observed_sigma(i,j,k), "adtge:",alphaf, deltaf, thetaf, gammaf, epsilonf, "A,lc:",max_blazef, lambda_cf,chi2
				end if
			else
				result_wav(i,j,k) = 0
				result_int(i,j,k) = 0
        ...
        ! chi2_all based on orders without orders after end_chi2 (from resimplex3.in)
        if ((rectifinal).and.(j<end_chi2)) then
                chi2_allA = chi2_allA + chi2_ord
        end if

end do ! j=n_ord

! printing every 10th iteration to chi2_funcA.tmp
...
end function chi2_funcA

end module chi2_funcA_module
